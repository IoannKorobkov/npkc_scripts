...
input_file = 'vtp.csv'   # замените на имя вашего файла
output_file = 'vtp_result.csv' # можно указать то же имя, если хотите перезаписать
...
with open(input_file, mode='r', encoding='utf-8') as infile:
    ...
    if 'task_pin_created_comment' not in fieldnames:
        raise ValueError("Столбец 'task_pin_created_comment' не найден в файле")
    ...
    with open(output_file, mode='w', encoding='utf-8', newline='') as outfile:
        ...
        for row in reader:
            ...
            comment = row['task_pin_created_comment']
            if comment is not None:
                ...
                row['task_pin_created_comment'] = comment
