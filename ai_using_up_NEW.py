...
Фаза 1 (VPN ON):  extract_phase()  — запросы к datalake, результат в _buffer
                   и на диск (_BUFFER_PKL), чтобы фазу 2 можно было повторять
                   отдельно без VPN, если она упадёт по сети.
Фаза 2 (VPN OFF): load_phase()     — вставка из _buffer в dwh_test_db батчами
                   по _INSERT_BATCH_SIZE строк (с ретраями на чанк).

Интегрируется в all_dashboards_up.py.
Глубина: DAYS_TO_SYNC (по умолчанию 3, задаётся из all_dashboards_up).
Повтор только загрузки после сбоя: python ai_using_up.py --load-only
"""
...
# Буфер между фазами — держится в памяти процесса, а также сохраняется на диск
# (extract_phase), чтобы фазу загрузки можно было повторять отдельно (--load-only,
# без VPN) до полного успеха, не гоняя фазу извлечения заново при каждом сбое сети.
_buffer: pd.DataFrame | None = None
_BUFFER_PKL = os.path.join(os.path.dirname(os.path.abspath(__file__)), '_ai_using_buffer.pkl')


# ===========================================================================
# VPN (CLI, trac.exe — Check Point Endpoint Connect; подменяются заглушками
# в all_dashboards_up.py)
# ===========================================================================
...
def _parse_trac_status(info_out: str) -> str:
    ...
    Наивный поиск 'idle' по всему тексту ловит чужой Idle-конн и пропускает
    зависший активный (напр. status: Connecting) — отсюда была ошибка
    'A previous connection is currently on progress'."""
    current_status = ''
...
def connect_vpn():
    print('[ai_using] Подключаю VPN через trac.exe...')
    send_ntfy_alert('Запускаю VPN для ai_using...', title='VPN Connect', priority='default', tags='lock')

    if _wait_for_dns(cfg.CH_HOST, timeout=1, interval=1):
        print(f"[ai_using] VPN уже поднят ('{cfg.CH_HOST}' резолвится) — пропускаю подключение.")
    ...
    # На этой машине сайт из cfg.VPN_SITE не совпадает с реально сконфигурированным ->
    # trac отвечает 'Connection could not be found'. Без -s trac сам использует
    # единственный настроенный (активный) сайт.
    ...
    if _wait_for_dns(cfg.CH_HOST, timeout=40, interval=3):
        print(f"[ai_using] DNS '{cfg.CH_HOST}' резолвится — туннель готов.")
    else:
        print(f"[ai_using] ⚠️ DNS '{cfg.CH_HOST}' не резолвится спустя 40с после подключения — пробую дальше как есть.")
...
def disconnect_vpn():
    print('[ai_using] Отключаю VPN через trac.exe...')
...
def _analyse_one(client, date_from: str, date_to: str, modality: str) -> pd.DataFrame:
    ...
    sql = f"""
        ...
        base_study AS (
            ...
            GROUP BY study_uid
            -- describe_start — алиас агрегата (min), поэтому фильтр по нему
            -- должен быть в HAVING, а не в WHERE
            HAVING describe_start >= '{MIN_DESCRIBE_START}'
        ),
        ai_result AS (
...
def extract_phase() -> bool:
    ...
    try:
        ...
        if total > 0:
            _buffer.to_pickle(_BUFFER_PKL)
            print(f'[ai_using] буфер сохранён на диск: {_BUFFER_PKL} '
                  f'(фазу загрузки можно повторять отдельно, --load-only)')
        return total > 0
    except Exception as e:
...
# Вставка батчами: один insert_df на весь исторический реюпload (2-4М строк)
# упирается в таймаут записи на целевой Yandex Cloud CH (см. падение 2026-09-03).
_INSERT_BATCH_SIZE = 100_000
_INSERT_MAX_RETRIES = 3
_INSERT_RETRY_DELAY = 10


def _insert_chunked(tc, table: str, df: pd.DataFrame, batch_size: int = _INSERT_BATCH_SIZE) -> None:
    total = len(df)
    for start in range(0, total, batch_size):
        chunk = df.iloc[start:start + batch_size]
        for attempt in range(1, _INSERT_MAX_RETRIES + 1):
            try:
                tc.insert_df(table, chunk)
                break
            except Exception as e:
                if attempt == _INSERT_MAX_RETRIES:
                    raise
                print(f'    {table}: попытка {attempt} не удалась ({e}), '
                      f'повтор через {_INSERT_RETRY_DELAY} сек...')
                time.sleep(_INSERT_RETRY_DELAY)
        print(f'    {table}: вставлено {min(start + batch_size, total)}/{total} строк')
...
def load_phase() -> bool:
    ...
    if (_buffer is None or len(_buffer) == 0) and os.path.exists(_BUFFER_PKL):
        print(f'[ai_using] буфер в памяти пуст — подхватываю сохранённый {_BUFFER_PKL}')
        _buffer = pd.read_pickle(_BUFFER_PKL)
    ...
    try:
        ...
        print(f'  -> {TABLE_BY_MODEL}: {len(upload_by_model)} строк (исследование×модель), '
              f'батчами по {_INSERT_BATCH_SIZE}')
        _insert_chunked(tc, TABLE_BY_MODEL, upload_by_model)
        print(f'  -> {TABLE_BY_STUDY}: {len(upload_by_study)} строк (1 на исследование), '
              f'батчами по {_INSERT_BATCH_SIZE}')
        _insert_chunked(tc, TABLE_BY_STUDY, upload_by_study)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load-only', action='store_true',
                        help=f'Пропустить VPN/извлечение, догрузить уже сохранённый буфер '
                             f'({_BUFFER_PKL}) — для повтора фазы 2 после сбоя сети без VPN')
    ...
    if args.load_only:
        if not os.path.exists(_BUFFER_PKL):
            print(f'[ai_using] ❌ --load-only: сохранённый буфер не найден ({_BUFFER_PKL})')
            sys.exit(1)
        print(f'[ai_using] --load-only: догружаю сохранённый буфер {_BUFFER_PKL}')
        ok = load_phase()
        try:
            dedup_check()
        except Exception as e:
            print(f'[ai_using] ⚠️ dedup_check упал после загрузки: {e} — '
                  f'данные уже вставлены, повторите dedup_check() отдельно позже')
        sys.exit(0 if ok else 1)

    if args.extract_only:
        print(f'[ai_using] --extract-only: только извлечение — последние {DAYS_TO_SYNC} дней')
        try:
            connect_vpn()
        except Exception as e:
            msg = f'[ai_using] Не удалось подключить VPN: {e}'
        ...
        if ok:
            print(f'[ai_using] ✅ извлечение сохранено в {_BUFFER_PKL}. '
                  f'Для загрузки: python ai_using_up.py --load-only')
        else:
            print('[ai_using] ❌ извлечение не удалось.')
        sys.exit(0 if ok else 1)
