...
_COLUMNS = [
    ...
    'llo_benefit_status', 'load_datetime', 'assignment_doctor_job_name', 'assignment_describe_start_date',
]
...
base AS (
    SELECT
        ...
        toTimeZone(toDateTime(t2.assignment_describe_start_date), 'Europe/Moscow') AS assignment_describe_start_date
...
SELECT 
    ...
    f.device_type,
    f.diagnostic_name,
    f.assessment_result_type_code,
    f.conduct_mo_name,
    f.conduct_mu_name,
    f.assignment_describe_mu_id,
    f.lpu_short_name_by_lpu_id,
    f.assignment_mu_id,
    f.lpu_short_name_by_assignment_mu_id,
    f.diag_code,
    f.norma_value,
    f.age_group,
    f.pin_status,
    f.task_pin_end_date,
    f.task_pin_status,
    f.llo_benefit_status,
    f.load_datetime,
    rd.assignment_doctor_job_name,
    f.assignment_describe_start_date
FROM filtered f
...
def _process_row(row: list) -> list:
    ...
    for i, val in enumerate(row):
        col = _COLUMNS[i]
        if col in ('conduct_date', 'describe_date', 'task_pin_end_date', 'load_datetime', 'assignment_describe_start_date'):
            nullable = col in ('describe_date', 'task_pin_end_date', 'assignment_describe_start_date')
            if val is None:
