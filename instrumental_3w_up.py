...
def export_instrumental_3w() -> bool:
    ...
    try:
        ...
        try:
            import dianet_parser_no_json_integrated

            df = dianet_parser_no_json_integrated.get_dianet_tasks_df(
                output=None,
                login_first=True
            )
            print(f"📥 [dianet] получено {len(df)} строк")

            dianet_parser_no_json_integrated.upload_df_to_clickhouse(df)
            print("✅ [dianet] dianet_parser обновлён")
        except Exception as e:
            print(f"⚠️ [dianet] не удалось обновить dianet_parser: {e}")
