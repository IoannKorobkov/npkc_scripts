...
_COLUMNS = [
    'report_date', 'diagnostic_code', 'diagnostic_name', 'modality', 'norma_value',
...
WITH studies AS (
    SELECT
        t2.accession_number,
        t2.diagnostic_code,
        any(t2.diagnostic_name) AS diagnostic_name,
        any(t2.device_type) AS modality,
        min(t2.assignment_describe_start_date) AS describe_start,
        min(t2.assignment_result_doc_created_date) AS describe_end,
        ai_res.norma_value AS norma_value
    FROM data_views.v_eris_assignment_results AS t2
    LEFT JOIN (
        SELECT
            JSONExtractString(raw_data, 'studyIUID') AS studyIUID,
            min(
                JSONExtractString(
                    JSONExtractString(raw_data, 'aiResult'),
                    'norma'
                )
            ) AS norma_value
        ...
        ON t2.study_uid = ai_res.studyIUID
    WHERE t2.assignment_describe_mu_id = %(mu_id)s
      AND t2.assignment_status = 'Выполнено'
      AND t2.assignment_describe_start_date IS NOT NULL
      AND t2.assignment_result_doc_created_date IS NOT NULL
      AND t2.accession_number IS NOT NULL
      AND t2.assignment_result_doc_created_date >= now() - INTERVAL {DAYS_TO_SYNC} DAY
    GROUP BY
        t2.accession_number,
        t2.diagnostic_code,
        ai_res.norma_value
...
def load_phase() -> bool:
    ...
    try:
        ...
        for row in raw_rows:
            report_date, diagnostic_code, diagnostic_name, modality, norma_value, n_studies, avg_seconds = row

            avg_time = _format_hms(avg_seconds) if avg_seconds is not None else '0:00:00'
            ...
            processed_rows.append([
                ...
                str(norma_value) if norma_value is not None else None,
